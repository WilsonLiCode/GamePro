...
GameApp::GameApp() 
{
	...
	screenShakeIntensity = 0.0001f;
	gameParticles = ParticleEmitter(30);
...
void GameApp::Init() 
{
	...
	displayWindow = SDL_CreateWindow("Dizzy Asteroids", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800, 600, SDL_WINDOW_OPENGL);
...
void GameApp::Render() 
{
	...
	{
		//Particles
		gameParticles.position.x = player.x;
		gameParticles.position.y = player.y;
		gameParticles.Render();

		//Shake screen
		Matrix tempMatrix;
		tempMatrix.identity();
		tempMatrix.m[3][0] = sin(screenShakeValue * screenShakeSpeed) * screenShakeIntensity;
		tempMatrix.m[3][1] = sin(screenShakeValue * screenShakeSpeed) * screenShakeIntensity;
	...
	{
		//easing
		float animationValue = mapValue(animationTime, 0.0f, 2.0f, 0.0f, 1.0f);

		glMatrixMode(GL_MODELVIEW);
		glPushMatrix();
		Matrix tempMatrix;
		tempMatrix.identity();
		tempMatrix.m[3][0] = -0.55f;
		tempMatrix.m[3][1] = easeOutElastic(-0.9f, 0.0f, animationValue);
		glMultMatrixf(tempMatrix.ml);
